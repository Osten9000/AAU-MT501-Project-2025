...
def Kalman_Filter_RLS(input_, measured_voltage, initial_x, initial_SigmaX, SigmaW, SigmaV, initial_theta, initial_sigmax_R, sigmav_R, xtore,lam):
    ...
    for k in range(1, maxIter):
        #gemme bedste state estimat fra sidste iteration
        if k==1:
            print(xhat)
        ...
        #1b
        SigmaW_t = np.matmul(np.matmul(B, SigmaW),B.T)
        SigmaX = np.matmul(np.matmul(A, SigmaX),A.T)  + SigmaW_t 
        ...
        # en iteration af klaman filter færdig z[k] = xhat[1, 0] (row 2 første konkolonne)
        
        #opdatere parameter med RLS, først vi finder vi V_RRC med målt output og z[k] estimering
        soc_target = np.round(xhatstore[1,k], 3)
        idx = np.argmin(np.abs(soc_data - soc_target))
        V_RRC_k = measured_voltage[k] - ocv_data[idx]
        
        
        soc_target_2 = np.round(xhatstore[1,k-1], 3)
        idx_2 = np.argmin(np.abs(soc_data - soc_target_2))
        V_RRC_k_minus = measured_voltage[k-1] - ocv_data[idx_2]
        
        #V_RRC_k_minus = xhat_minus[0,0]
        """
...
SigmaW = np.eye(2) * measure_error
SigmaV = measure_error_2
soc=0.7
...
ax1.legend()
plt.savefig('disk, rette sigmaer.pdf', format='pdf', dpi=300, bbox_inches='tight')
ax1.grid(True)
